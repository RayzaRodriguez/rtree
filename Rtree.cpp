...
bool Rtree::findpointleaf (node**& p,point d)
{
    ...
    if(possibleleafs.size()==0)
        return 0;
    for(unsigned int j=0;j<possibleleafs.size();j++)
    {
        p=possibleleafs[j];
...
//indice next, nido
void Rtree::picknext (int& ii, node** p, vector<node*>&temp,node* l, node* r,float& d1f,float& d2f)
{
    float difference=0;
    float d1,d2;
    ii=0;
...
void Rtree::pickseeds (int& ii, int& jj ,node** p)
{
    ...
    jj=1;
    for(unsigned int i=0;i<(*p)->children.size();++i){
        for(unsigned int j=i+1; j<(*p)->children.size();++j)
        {
...
float Rtree :: distance_points (point A, point B)
{
    float x = pow(A.first-B.first,2);
    float y = pow(A.second-B.second,2);
    return sqrt(x+y);
}
float Rtree :: area (point cmin, point cmax)
{
    return fabs((cmax.first-cmin.first)*(cmax.second-cmin.second));
}

float Rtree :: distance_rec_point(point d,point cmin, point cmax)
{
